...
The MSK family is AWS-specific. GCP has a Managed Service for Apache Kafka (via GCP managed Kafka), and Azure has Event Hubs with Kafka-compatible endpoints. Each provider will have its own family design (e.g., `GCPManagedKafkaConfig` in `gcp.kropath.run`, `AzureEventHubsConfig` in `azure.kropath.run`) when `kropath-gcp` and `kropath-azure` are bootstrapped. Field names and governance structures may differ due to provider-specific capabilities.
